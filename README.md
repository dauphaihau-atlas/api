...
- **Idempotency Keys** — Client-supplied `Idempotency-Key` header prevents duplicate import jobs and export triggers; keys are scoped by tenant + user; duplicate-safe with a DB unique constraint
...
- **Events & Listeners** — Domain events (`UserCreated`, `ImportChunkProcessed`, `ImportCompleted`) decoupled from side-effects via dedicated listener classes (`SendUserCreatedNotification`, `BroadcastImportProgress`, `BroadcastImportCompleted`) registered in `EventServiceProvider`
...
| **Idempotency** | `IdempotencyMiddleware` stores a SHA-256 scope/request hash in `idempotency_keys` and replays cached responses for duplicate requests; race-safe via DB unique constraint |
...
|---|---|---|---|---|
| `POST` | `/v1/users/import` | ✓ admin | Required | Upload CSV, start async batch processing |
| `GET` | `/v1/users/import/{id}/status` | ✓ | — | Real-time import progress |
| `DELETE` | `/v1/users/import/{id}` | ✓ admin | — | Cancel in-progress import |
| `GET` | `/v1/users/export` | ✓ admin | Required | Trigger CSV export, returns signed URL |
...
Prevents duplicate operations when clients retry after a timeout or network error.

**Which endpoints require a key**: `POST /v1/users/import` and `GET /v1/users/export`.
...
- Whitespace, slashes, quotes, and control characters are rejected (422)
- Generated by the client — treat as opaque; the server does not parse meaning from the value

**Retry behavior**

| Scenario | Response |
|---|---|
| Same key, same request payload | Original response replayed with `Idempotency-Replayed: true` header |
| Same key, different payload | `409 Conflict` with `error_code: IDEMPOTENCY_CONFLICT` |
| Missing key on required endpoint | `422` with `error_code: MISSING_IDEMPOTENCY_KEY` |
| Invalid key format | `422` with `error_code: INVALID_IDEMPOTENCY_KEY` |

Keys expire after 24 hours (configurable via `IDEMPOTENCY_TTL_HOURS`). Scope is per `(tenant_id, user_id, endpoint, key)` — the same key value is independent across users and tenants.

**Example request**

```http
POST /api/v1/users/import HTTP/1.1
Authorization: Bearer <token>
X-Tenant-ID: my-tenant
Idempotency-Key: import-2026-04-27-batch-001
...
1. Upload CSV via `POST /v1/users/import` (include `Idempotency-Key`)
...
php artisan app:prune-idempotency-keys   # Expired idempotency key records (runs hourly via scheduler)
```
...
| `idempotency_keys` | Idempotency records scoped by tenant + user (TTL 24 h) |
...
# Idempotency key TTL (hours, default 24)
